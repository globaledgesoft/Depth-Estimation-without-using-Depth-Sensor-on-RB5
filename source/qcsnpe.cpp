...
Qcsnpe::Qcsnpe(std::string &dlc, std::vector<std::string> &output_layers, int system_type) {
    ...
    std::cout<<"snpe object created"<< std::endl;
}
...
std::map<std::string, std::vector<float>> Qcsnpe::predict(py::array_t<uint8_t> img) {
    ...
    if(in_size != img_size) {
        std::cout<<"in_size "<<in_size<<"img_size"<<img_size<<std::endl;
    ...
    for(auto i=0; i<img_size; i++) {
        tensor_ptr_fl[i] = static_cast<float>(input_image.data[i]/255.0);
    ...
    for(size_t i=0; i<out_tensors.size(); i++) {
        ...
        //std::cout<<std::string(out_tensors.at(i))<<std::endl;
    }
